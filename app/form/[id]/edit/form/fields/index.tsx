...
export function FormFields({ id, defaultRequired }: { id: string, defaultRequired: boolean }) {
    ...
    if (active.id !== over?.id && formFields) {
      ...
      newOrder.forEach((field, index) => {
        if (field.order !== index + 1) {
          updateField({ fieldId: field._id, order: index + 1, formId: id as Id<"forms"> });
        }
      });
    }
  }
  ...
  function handleTypeChange(type: string): void {
    if (selectedItem) {
      updateField({ fieldId: selectedItem, type: type as FieldType, formId: id as Id<"forms"> });
    }
  }
          ...
          {formFields &&
            formFields.map((field) => (
              <FieldCard
                ...
                onTypeChange={(newType) => updateField({ fieldId: field._id, type: newType as FieldType, formId: id as Id<"forms">})}
                editingFieldName={selectedItem === field._id ? editingFieldName : null}
                onEditingFieldNameChange={selectedItem === field._id ? setEditingFieldName : () => {}}
                onSaveFieldName={handleSaveFieldName}
                onUpdateOptions={(options: {name: string, order: number}[]) => updateField({ fieldId: field._id, selectOptions: options, formId: id as Id<"forms">})}
              />
            ))}
          <Button className=" w-full h-10 bg-transparent border-dashed border-2 border-gray-300" variant="outline" onClick={createField}>
