...
import { FieldContent, ChoiceFieldContent } from "./fieldContent";
...
export function FieldCard({ 
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      <Card onClick={onSelect} className={`${isActive ? "border-2 border-primary" : ""} w-full`}>
        <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 p-4">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <Button variant="ghost" size="icon" {...listeners} className="cursor-grab flex-shrink-0">
            ...
            {isActive ? (
              ...
              <CardTitle className="text-lg truncate max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg">{field.name}</CardTitle>
            )}
          </div>
        ...
        <CardContent className="p-4 pt-0">
          {isActive ? (
            isChoiceField ? (
              <ChoiceFieldContent field={field} onUpdateOptions={onUpdateOptions} />
            ) : (
            ...
            isChoiceField ? (
                <ChoiceFieldContent field={field} onUpdateOptions={onUpdateOptions} /> // Or a dedicated preview component
            ) : (
